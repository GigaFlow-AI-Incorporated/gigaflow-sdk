...
Opens the backend's interactive Flow viewer in the browser.  The viewer shows
span timelines always; Flow tabs (Orchestration, Atomic View, Metrics) are shown
when Flow has been computed and are replaced with a "Compute Flow" prompt otherwise.
...
def register(sub) -> None:
    p = sub.add_parser(
        "inspect",
        help="Open the interactive Flow viewer for a trace",
    )
...
def _handle_inspect(args, base_url: str) -> None:
    ...
    if args.cli:
        ...
        viewer_url = f"{base_url.replace('/api/v1', '')}/flow/{args.trace_id}"
