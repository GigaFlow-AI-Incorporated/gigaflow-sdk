"""compute command — batch Flow computation over a SQL-selected set of traces.
...
def register(sub) -> None:
    p = sub.add_parser(
        "compute",
        help="Batch-compute Flow for traces matching a SQL query",
        description=(
            "Run Flow analysis on every trace returned by a SQL SELECT.\n\n"
            "The query must return a `trace_id` column. Traces that already have Flow\n"
    ...
    p.add_argument(
        "--force",
        action="store_true",
        help="Recompute Flow even for traces that already have results",
    )
    p.add_argument(
        "--concurrency",
        type=int,
...
def _handle_compute(args, base_url: str) -> None:
    ...
    if args.force:
        ...
        if skipped:
            _fmt.info(f"Skipping {len(skipped)} trace(s) with existing Flow results (use --force to recompute)")
        if not trace_ids:
            _fmt.ok("All traces already have Flow results — nothing to do.")
            return

    _fmt.section(f"Computing Flow for {len(trace_ids)} trace(s)  [concurrency={args.concurrency}]")
...
def _partition_computed(
    ...
    # Build a query to find which of the given IDs already have Flow (run_id IS NOT NULL)
...
def _run_one(
    ...
    """Run Flow for a single trace. Returns (groundedness, tool_consumption, token_usage).

    ``token_usage`` is the raw dict as serialised by ``FlowTokenUsage.model_dump``:
