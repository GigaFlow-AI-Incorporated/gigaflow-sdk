...
The full design lives in [`docs/superpowers/specs/2026-04-22-tool-authoritativeness-design.md`](https://github.com/GigaFlow-AI-Incorporated/gigaflow/blob/main/docs/superpowers/specs/2026-04-22-tool-authoritativeness-design.md) in the main GigaFlow repo. This section is the user-facing how-to.
...
2. **Run Flow on a known trace.** `gigaflow compute "SELECT trace_id FROM trace_metrics WHERE trace_id = '<your-id>'" --force`.
...
4. **Check the rolled-up metric.** `authoritative_groundedness` appears in the Flow viewer's Metrics tab and in `trace_metrics`. `null` means the trace was scored but no atom was stamped above the prior — usually a sign that no `auth_mappings` rule fired.
