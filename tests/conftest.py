...
class _MockAPIHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if p == "/api/v1/projects/":
            ...
            _MockAPIHandler.last_datasource_auth_header = self.headers.get("Authorization")
        ...
        elif p == f"/api/v1/flow/{MOCK_TRACE_ID}":
            try:
                _MockAPIHandler.last_flow_body = json.loads(raw) if raw else {}
            except Exception:
                _MockAPIHandler.last_flow_body = {}
            _MockAPIHandler.last_flow_headers = dict(self.headers.items())
