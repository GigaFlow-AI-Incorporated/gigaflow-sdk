...
class TestDefaultTransformContent:
    """The built-in YAML must have the filter fields and content mappings that Flow needs."""
    ...
    def test_llm_call_maps_completion(self, raw):
        """Flow reads primitive_data['completion'] to get the LLM response text."""
        assert "completion:" in raw

    def test_tool_invocation_maps_tool_output(self, raw):
        """Flow reads primitive_data['tool_output'] to get the tool result text."""
        assert "tool_output:" in raw

    def test_user_input_maps_content(self, raw):
        """Flow reads primitive_data['content'] to extract the user query."""
...
class TestLogfireDefaultTransform:
    ...
    ``primitive_type=NULL`` and every downstream Flow groundedness / tool
