...
class TestCompute:
    def test_compute_runs_flow_on_matching_traces(self, installed_cli, mock_server, configured_env_with_key):
        """compute runs Flow for each trace returned by the SQL query."""
        result = run(
    ...
    def test_compute_shows_progress(self, installed_cli, mock_server, configured_env_with_key):
        ...
        assert result.returncode == 0, err(result)
        output = out(result)
        assert "Computing Flow" in output
        assert "1/1" in output

    def test_compute_forwards_api_key(self, installed_cli, mock_server, configured_env_with_key):
        """OPENAI_API_KEY must be forwarded as api_key in the Flow POST body."""
        ...
        assert result.returncode == 0, err(result)
        assert _MockAPIHandler.last_flow_body.get("api_key") == "sk-test-key-from-env"
    ...
    def test_compute_passes_model_and_k_threshold(self, installed_cli, mock_server, configured_env_with_key):
        """--model and --k-threshold are forwarded to the Flow endpoint."""
        _MockAPIHandler.last_flow_body = {}
        result = run(
            ["--backend", mock_server, "compute",
             "SELECT trace_id FROM trace_metrics",
        ...
        assert result.returncode == 0, err(result)
        assert _MockAPIHandler.last_flow_body.get("model") == "gpt-4o"
        assert _MockAPIHandler.last_flow_body.get("k_threshold") == 0.5
